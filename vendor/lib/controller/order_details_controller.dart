...
class OrderDetailsController extends GetxController {
    ...
    if (argumentData != null) {
      ...
      if (orderModel.value.taxSetting != null) {
        for (var element in orderModel.value.taxSetting!) {
          taxAmount.value =
              taxAmount.value + Constant.calculateTax(amount: (subTotal.value - double.parse(orderModel.value.discount.toString()) - specialDiscount.value).toString(), taxModel: element);
        }
      }
      ...
      if (orderModel.value.adminCommissionType == 'Percent') {
        double basePrice = subTotal.value / (1 + (double.parse(orderModel.value.adminCommission!) / 100));
  ...
  Future<List<int>> getTicket() async {
    ...
    bytes += generator.text('${'Phone:'.tr} ${orderModel.value.vendor!.phonenumber}', styles: const PosStyles(align: PosAlign.center));
    bytes += generator.text('TIN/VAT No.: xxxxxxxxxxxxx'.tr, styles: const PosStyles(align: PosAlign.center));
    ...
    bytes += generator.text(
      '${'Date:'.tr} ${DateFormat('MMM d yyyy, h:mm:ss a').format(DateTime.fromMicrosecondsSinceEpoch(orderModel.value.vendor!.createdAt!.microsecondsSinceEpoch)).toString()} ',
