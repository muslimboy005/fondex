...
class FireStoreUtils {
  ...
  static Future<List<CarMakes>> getCarMakes() async {
    ...
    await fireStore
        .collection(CollectionName.carMake)
        .where('isActive', isEqualTo: true)
        .get()
        .then((value) {
          for (var element in value.docs) {
  ...
  static Future getPaymentSettingsData() async {
    ...
    // Payme
    log(
      '🔵 [FireStoreUtils.getPaymentSettingsData] Payme settings o\'qilmoqda...',
    );
    await fireStore
        .collection(CollectionName.settings)
        .doc("paymeSettings")
  ...
  static Future<void> updateWallateAmount(OrderModel orderModel) async {
    ...
    await FireStoreUtils.setWalletTransaction(deliveryChargeModel).then((
      value,
    ) async {
      if (value == true) {
        await FireStoreUtils.updateUserWallet(
  ...
  static Future<ChatVideoContainer?> uploadChatVideoToFireStorage(
    ...
    try {
      ...
      ShowToastDialog.closeLoader();
      ShowToastDialog.showToast("${'Error'.tr}: ${e.toString()}");
      return null;
    }
  }
  ...
  static Future<List<ParcelOrderModel>> getParcelDriverOrdersOnce(
    ...
    return snapshot.docs
        .map(
          (doc) =>
              ParcelOrderModel.fromJson(doc.data() as Map<String, dynamic>),
        )
        .toList();
  }

  static Future<List<RentalOrderModel>> getRentalDriverOrdersOnce(
    String driverId,
  ) async {
    Query query = fireStore
        .collection(CollectionName.rentalOrders)
    ...
    return snapshot.docs
        .map(
          (doc) =>
              RentalOrderModel.fromJson(doc.data() as Map<String, dynamic>),
        )
        .toList();
  }
