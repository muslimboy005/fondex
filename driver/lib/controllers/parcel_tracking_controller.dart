...
class ParcelTrackingController extends GetxController {
  ...
  void _listenToOrder() {
    orderSubscription = FireStoreUtils.fireStore
      ...
      if (event.data() != null) {
        ...
        if (orderModel.value.driverId != null) {
          driverSubscription = FireStoreUtils.fireStore
  ...
  void _updateGoogleMap() async {
    ...
    final result = await polylinePoints.getRouteBetweenCoordinates(
      request: PolylineRequest(
        origin: PointLatLng(driverLat, driverLng),
        destination: PointLatLng(dstLat??0.0, dstLng),
        mode: TravelMode.driving,
      ),
    );
    ...
    if (orderModel.value.status == Constant.driverAccepted) {
      _addGoogleMarker(
          orderModel.value.senderLatLong?.latitude?.toDouble(),
          orderModel.value.senderLatLong?.longitude?.toDouble(),
          "Pickup",
          pickupIcon!);
    } else {
      _addGoogleMarker(
          orderModel.value.receiverLatLong?.latitude?.toDouble(),
          orderModel.value.receiverLatLong?.longitude?.toDouble(),
          "Dropoff",
          dropoffIcon!);
    }
  ...
  void _addGoogleMarker(
      double? lat, double? lng, String id, BitmapDescriptor icon) {
    if (lat == null || lng == null) return;
    markers[MarkerId(id)] = Marker(
      markerId: MarkerId(id),
      position: LatLng(lat, lng),
