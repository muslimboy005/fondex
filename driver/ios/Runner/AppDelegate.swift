...
@objc class AppDelegate: FlutterAppDelegate, PKPushRegistryDelegate {
  ...
  // disable VoIP for this app. flutter_callkit_incoming handles this once
  // we forward the payload here.
  func pushRegistry(_ registry: PKPushRegistry,
                    didReceiveIncomingPushWith payload: PKPushPayload,
                    for type: PKPushType,
                    completion: @escaping () -> Void) {
    let data = payload.dictionaryPayload
    let orderId = (data["orderId"] as? String) ?? (data["rideId"] as? String) ?? (data["id"] as? String) ?? UUID().uuidString
    ...
    callKitData.appName = "Fondex Driver"
