...
class DineInRestaurantDetailsController extends GetxController {
  ...
  Future<void> orderBook() async {
    ...
    try {
      ...
      // Vaqt formatlashni xavfsiz qilish
      String hour = "18:00"; // Default qiymat
      if (selectedTimeSlot.value.isNotEmpty) {
        try {
          final timeString = (Intl.getCurrentLocale() == "en_US") 
              ? selectedTimeSlot.value 
              : selectedTimeSlot.value.toLowerCase();
          
          // Vaqt formatini tekshirish va parse qilish
          final parsedTime = DateFormat('hh:mm a').parse(timeString);
          hour = DateFormat("kk:mm").format(parsedTime);
        } catch (e) {
          print("Error parsing time: $e, using default time");
          // Agar parse qilishda xato bo'lsa, default vaqt ishlatish
          hour = "18:00";
        }
      }
      
      // Hour'ni parse qilish
      final hourParts = hour.split(":");
      if (hourParts.length == 2) {
        final hourInt = int.tryParse(hourParts[0]) ?? 18;
        final minuteInt = int.tryParse(hourParts[1]) ?? 0;
        dt = DateTime(dt.year, dt.month, dt.day, hourInt, minuteInt, dt.second, dt.millisecond, dt.microsecond);
      } else {
        // Default vaqt
        dt = DateTime(dt.year, dt.month, dt.day, 18, 0, dt.second, dt.millisecond, dt.microsecond);
      }
    ...
    DineInBookingModel dineInBookingModel = DineInBookingModel(
      ...
      // Background operatsiyalar - notification
      _sendNotificationInBackground();
  ...
  void getRecord() {
    ...
    if (dateList.isNotEmpty) {
      ...
      // Vaqt slot'ni xavfsiz formatlash
      if (timeSlotList.isNotEmpty && timeSlotList[0].time != null) {
        try {
          selectedTimeSlot.value = DateFormat('hh:mm a').format(timeSlotList[0].time!);
        } catch (e) {
          print("Error formatting time slot: $e");
          // Default vaqt
          selectedTimeSlot.value = '6:00 PM';
        }
      } else {
        // Default vaqt
        selectedTimeSlot.value = '6:00 PM';
      ...
      // Agar dateList bo'sh bo'lsa, default qiymatlar
      selectedTimeSlot.value = '6:00 PM';
    }
  }
