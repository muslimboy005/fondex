    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          log("   Role Value: '$role'");
          log("   Role Check: ${role.toString().toLowerCase() == "customer"}");
          ...
          // Check if role is not customer and not empty
          final roleLower = role.toString().toLowerCase();
          if (roleLower.isNotEmpty && roleLower != "customer") {
            log("❌ VALIDATION FAILED: Role is not 'customer' and not empty");
            log("   Expected: 'customer' or empty string");
            log("   Got: '$role'");
            Get.snackbar(
              "Error".tr,
  ...
  Future<void> _loginWithFirebase(String email, String password) async {
    ...
    try {
      ...
      if (userModel.shippingAddress?.isNotEmpty ?? false) {
        ...
        log("   Navigating to Service List Screen...");
        Get.offAll(() => const ServiceListScreen());
      } else {
        log("⚠️ No shipping address found");
        log("   Navigating to Location Permission Screen...");
        Get.offAll(() => const LocationPermissionScreen());
      }
      log("✅ Navigation completed");
    } catch (e, stackTrace) {
